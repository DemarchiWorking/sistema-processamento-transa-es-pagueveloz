...
namespace PagueVeloz.CoreFinanceiro.Aplicacao.Comandos
{
    public class ProcessarCapturaCommandHandler : IRequestHandler<ProcessarCapturaCommand, TransacaoResponse>
    {
        ...
        private readonly ILogger<ProcessarCapturaCommandHandler> _logger;
        ...
        public ProcessarCapturaCommandHandler(
            ...
            ILogger<ProcessarCapturaCommandHandler> logger)
        {
        ...
        public async Task<TransacaoResponse> Handle(ProcessarCapturaCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                conta.Capture(request.Amount, request.ReferenceId);
                ...
                {
                    _logger.LogWarning(ex, "Conflito de concorrência na captura: {ReferenceId}", request.ReferenceId);
                    throw new ConflitoConcorrenciaException("Conflito de concorrência detectado durante a captura.", ex);
                }
            ...
            {
                _logger.LogWarning(ex, "Falha de negócio na captura: {ReferenceId}", request.ReferenceId);
                return FailResponse(ex.Message, request.ReferenceId, conta);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erro crítico ao processar captura: {ReferenceId}", request.ReferenceId);
                return FailResponse("Erro interno ao processar a transação.", request.ReferenceId, conta);
            }
        }
